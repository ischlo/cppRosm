...
struct invalid_location : public std::range_error {
  ...
  explicit invalid_location(const std::string &what) : std::range_error(what) {}

  explicit invalid_location(const char *what) : std::range_error(what) {}

}; // struct invalid_location

namespace detail {

enum { coordinate_precision = 10000000 };
...
inline int32_t string_to_location_coordinate(const char **data) {
  ...
  if (*str != '.') {
    ...
    if (*str >= '0' && *str <= '9') {
      ...
      throw invalid_location{std::string{"wrong format for coordinate: '"} +
                             full + "'"};
    }
...
inline T append_location_coordinate_to_string(T iterator, int32_t value) {
  ...
  char *t = temp;
  do {
    *t++ = static_cast<char>(v % 10) +
           '0'; // NOLINT(bugprone-narrowing-conversions,
...
public:
  ...
  explicit constexpr Location() noexcept
      : m_x(undefined_coordinate), m_y(undefined_coordinate) {}
  ...
  constexpr Location(const int64_t x, const int64_t y) noexcept
      : m_x(static_cast<int32_t>(x)), m_y(static_cast<int32_t>(y)) {}
  ...
  Location(const double lon, const double lat)
      : m_x(double_to_fix(lon)), m_y(double_to_fix(lat)) {}
  ...
  constexpr bool valid() const noexcept {
    return m_x >= -180 * precision() && m_x <= 180 * precision() &&
           m_y >= -90 * precision() && m_y <= 90 * precision();
  }
  ...
  constexpr int32_t x() const noexcept { return m_x; }

  constexpr int32_t y() const noexcept { return m_y; }
   ...
   */
  double lon_without_check() const noexcept { return fix_to_double(m_x); }
   ...
   */
  double lat_without_check() const noexcept { return fix_to_double(m_y); }
  ...
  Location &set_lon(const char *str) {
    ...
    if (**data != '\0') {
      throw invalid_location{std::string{"characters after coordinate: '"} +
                             *data + "'"};
    }
    m_x = value;
    return *this;
 ...
 */
inline constexpr bool operator==(const Location &lhs,
...
template <int N> inline size_t hash(const osmium::Location &location) noexcept {
  return static_cast<uint32_t>(location.x()) ^
         static_cast<uint32_t>(location.y());
}

template <> inline size_t hash<8>(const osmium::Location &location) noexcept {
...
// #endif
template <> struct hash<osmium::Location> {
